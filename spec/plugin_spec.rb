...
describe Ruber::Plugin do
  ...
  before :all do
    class << Ruber[:main_window]
      def remove_about_plugin_action *args
      end
    end
  end
  ...
  after do
    Ruber[:components].class.class_eval{signals :unloading_test}
    unless @already_unloaded
      Ruber[:components].unload_plugin :test rescue nil
    end
  end
  ...
  describe 'Ruber::Plugin, when created' do
    ...
    it 'should call super passing the application object as argument' do
      ...
      plug.parent.should equal(Ruber[:app])
    end
    ...
    it 'should store itself in the component manager' do
      flexmock(Ruber[:components]).should_receive(:add).once .with(Ruber::Plugin)
    ...
    it 'should connect its "load_settings()" slot to the configuration manager\'s "settings_changed()" signal' do
      ...
      Ruber[:config].instance_eval{emit settings_changed}
    end
    ...
    it 'should use the plugin\'s binding to evaluate the options\' default value' do
      ...
      flexmock(Ruber[:config]){|m| m.should_receive(:add_option).once.with(OS.new(:name => :o1, :group => :G, :default => 7, :relative_path => false, :eval_default => true, :order => nil))}
      plug = Ruber::Plugin.new Ruber::PluginSpecification.full(@data)
      class Ruber::Plugin
    ...
    it 'should not attempt to register its options if the config component doesn\'t exist' do
      comp = Ruber[:components]
      def comp.[] name
        name == :config ? nil : super(name)
      end
    ...
    it 'should add all the config widgets it provides, if any, to the config manager' do
      ...
      old_widgets = Ruber[:config].dialog.widgets
      Ruber::Plugin.new Ruber::PluginSpecification.full(@data)
      widgets = Ruber[:config].dialog.widgets
      new_widgets = widgets.reject{|w| old_widgets.any?{|ow| ow.class == w.class}}
      new_widgets.size.should == 2
      new_widgets.any?{|w| w.is_a?(Qt::CheckBox)}.should_not be_nil
      new_widgets.any?{|w| w.is_a?(Qt::PushButton)}.should_not be_nil
    end
    ...
    it 'should call the "load_settings" method, if the configuration manager exists' do
      ...
      @data[:config_options]=  { :General=> { :o1 => {:type => 'string', :default => '"#{\'a\'.capitalize}"'} } }
      flexmock(Ruber[:config]).should_receive(:add_option)
      plug = Ruber::Plugin.new Ruber::PluginSpecification.full(@data)
    ...
    it 'should create the tool widgets mentioned in the PDF' do
      ...
      flexmock(Ruber[:main_window]).should_receive(:add_tool).with(:bottom, Qt::CheckBox, pix1, 'w1').once
      flexmock(Ruber[:main_window]).should_receive(:add_tool).with(:left, Qt::LineEdit, pix2, 'w2').once
  ...
  describe 'Ruber::Plugin#unload' do
    ...
    it 'should remove all tool widgets from the main window' do
      ...
      flexmock(Ruber[:main_window]).should_receive(:remove_tool).once.with('w1')
      flexmock(Ruber[:main_window]).should_receive(:remove_tool).once.with('w2')
      flexmock(Ruber[:main_window]).should_receive(:remove_tool).once.with('w3')
      plug = Ruber::Plugin.new Ruber::PluginSpecification.full(@data)
      plug.unload
    end
    ...
    it 'shouldn\'t attempt tp remove the configuration options from the configuration manager, if it doesn\'t exist' do
      options = {:x => {:a => {:default => 3}, :b => {:default => 'string'}}, :y => {:c => {:default => true}}}
      #Remove all options which can have been added in previous examples
      options.each_pair do |grp, v|
        v.each_key do |name|
          Ruber[:config].remove_option(grp, name)
        end
      end
      Ruber[:config].remove_option :x, :a
      @data[:config_options] = options
      plugin = Ruber::Plugin.new Ruber::PluginSpecification.full(@data)
    ...
    describe '#add_options_to_project' do
      ...
      before do
        @data[:project_options] = {
          ...
          :g2 => {:o1 => {:default => %w[a b c], :scope => :document}, :o3 => {:default => :xyz}}
        }
        @plug = Ruber::Plugin.new Ruber::PluginSpecification.full(@data)
        @prj = Ruber::Project.new('test.ruprj', 'Test')
        [[:g1, :o1], [:g2, :o1], [:g2, :o3]].each{|opt| @prj.remove_setting *opt}
      end
      
      after do
        class << Ruber[:components]
          signals :unloading_test
        end
        Ruber[:components].unload_plugin :test
      end
      ...
      it 'uses the project\'s binding to evaluate the default values of the options' do
        ...
        @plug.plugin_description.project_options[[:g1,:o1]].default = 'self.project_file'
        @plug.add_options_to_project @prj
        opts = @prj.instance_variable_get(:@known_options)
        opts[[:g1,:o1]].default.should == @prj.project_file
      end
      ...
      it 'ignores any option which has already been added if the second parameter is false' do
        ...
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g1, :o1]]).and_return true
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g1, :o2]]).and_return true
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g2, :o1]]).and_return true
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g2, :o3]]).and_return true
...
#       
#       before :all do
#         class LE < Qt::LineEdit; def initialize prj=nil;super nil;end;end
#         class CB < Qt::ComboBox; def initialize prj=nil;super nil;end;end
#         class ChB < Qt::CheckBox; def initialize prj=nil;super nil;end;end
#       end
#       
#       before do
#         @data[:project_widgets] = [
#           {:class => 'CB', :caption => 'C1', :scope => :document},
#           {:class => 'LE', :caption => 'C1', :scope => :global},
#           {:class => 'ChB', :caption => 'C2'},
#         ]
#         @plug = Ruber::Plugin.new Ruber::PluginSpecification.full(@data)
#         @prj = Ruber::Project.new('test.ruprj', 'Test')
#       end
...
#         old_widgets = @prj.dialog.widgets
#         @plug.register_with_project @prj
#         le = LE.new
#         cb = ChB.new
#         flexmock(LE).should_receive(:new).once.and_return(le)
#         flexmock(ChB).should_receive(:new).once.and_return(cb)
#         widgets = @prj.dialog.widgets
#         p old_widgets.map{|w| w.class}
#         p widgets.map{|w| w.class}
#         new_widgets = widgets.reject{|w| old_widgets.any?{|ow| ow.class == w.class}}
#         new_widgets.size.should == 2
#         new_widgets.find{|w| w.is_a? Qt::ComboBox}.should be_nil
#         new_widgets.find{|w| w.is_a? Qt::LineEdit}.should_not be_nil
#         new_widgets.find{|w| w.is_a? Qt::CheckBox}.should_not be_nil
#       end
#       
    end
    ...
    describe '#remove_options_from_project' do
        ...
        before do
          ...
          @plug = Ruber::Plugin.new Ruber::PluginSpecification.full(@data)
          @prj = Ruber::Project.new('test.ruprj', 'Test')
          flexmock(Ruber[:components]).should_receive(:each_component).and_yield(@plug)
        end
      ...
      it 'removes all options belonging to the plugin for which the project\'s match_rule? method returns true if the second argument is true' do
        ...
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g1, :o1]]).and_return true
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g1, :o2]]).and_return false
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g2, :o1]]).and_return true
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g2, :o3]]).and_return true
        @plug.remove_options_from_project @prj, true
        @prj.should_not have_option(:g1, :o1)
        @prj.should_not have_option(:g2, :o1)
        @prj.should_not have_option(:g2, :o3)
      end
      ...
      it 'removes all options belonging to the plugin and previously added to the project for which the project\'s match_rule? method returns false if the second argument is false' do
        desc = @plug.plugin_description
        @prj.add_option OS.new({:group => :g3, :name => :o3, :default => /a/})
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g1, :o1]]).and_return true
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g2, :o1]]).and_return false
        flexmock(@prj).should_receive(:match_rule?).with(desc.project_options[[:g2, :o3]]).and_return true
        @plug.remove_options_from_project @prj, false
        @prj.should_not have_option(:g2, :o1)
        @prj.should have_option(:g1, :o1)
        @prj.should have_option(:g2, :o3)
        @prj.should have_option(:g3, :o3)
      end
    ...
    describe '#remove_extensions_from_project' do
      ...
      before do
        @cls_name = self.class.const_get(:Ext).name
        @data[:extensions] = {
                          :ext1 => {:class => @cls_name, :scope => :global},
                          :ext2 => {:class => 'Qt::Object', :scope => :document},
                          :ext3 => {:class => @cls_name},
                          }
        @plug = Ruber::Plugin.new Ruber::PluginSpecification.full(@data)
      ...
      after do
        Ruber[:components].class.class_eval do
          signals :unloading_plug1 unless instance_methods.include? :unloading_plug1
        end
        Ruber[:components].unload_plugin :plug1
      end
      ...
      describe ', when the second argument is true' do
        ...
        it 'removes all extensions provided by the plugin for which the project\'s match_rule? method returns true' do
          @prj.extensions.keys.should =~ [:e1, :e2, :ext1, :ext3, :environment]
          @plug.remove_extensions_from_project @prj, true
          @prj.extensions.keys.sort.should =~ [:e1, :e2, :environment]
        end
        ...
        it 'removes a multiple extension from the project if one of the entries\' rule matches the project\'s, when the second argument is true' do
          ...
          @plug.remove_extensions_from_project @prj, true
          @prj.extensions.keys.should =~ [:e1, :e2, :environment]
        end
...
#           m.should_receive(:test).once.with('ext4', @prj).ordered('4')
      ...
      describe ', when the second argument is false' do
        ...
        it 'removes a multiple extension only if the class of the existing extension corresponds to the one specified by the entry for which the project\'s match_rule? fails' do
          ...
          @plug.remove_extensions_from_project @prj, false
          @prj.extensions.keys.should =~ [:e1, :e2, :ext1, :ext3, :ext4, :environment]
        end
...
#           flexmock(@prj).should_receive(:remove_extension).with(:ext5).once.ordered('5')
          ...
          @plug.connect(SIGNAL('extension_removed(QString, QObject*)'))do |s, o|
            @prj.extensions.should_not include(s.to_sym)
            m.test s, o
          end
          @plug.remove_extensions_from_project @prj, false
        end
